...
\input{../tables/tex/comps/company_overview}
...
\input{../tables/tex/comps/expectation_metrics}
...
\input{../tables/tex/comps/capitalization}
...
\input{../tables/tex/ma/revenue_synergy_valuation}
...
\input{../tables/tex/ma/has_gets_analysis}
...
\input{../tables/tex/ma/comparable_transactions}
...
The analysis pipeline follows four sequential steps:

\begin{enumerate}
    \item \textbf{Data Extraction} (\texttt{extract\_data.py}) $\rightarrow$ \texttt{master\_financials.json}
    \item \textbf{Ratio Calculation} (\texttt{calculate\_*.py}) $\rightarrow$ \texttt{comprehensive\_*.csv}
    \item \textbf{Table Conversion} (\texttt{csv\_to\_latex.py}) $\rightarrow$ \texttt{tables/tex/*.tex}
    \item \textbf{LaTeX Compilation} (\texttt{pdflatex}) $\rightarrow$ \texttt{main.pdf}
\end{enumerate}

Python scripts connect directly to SEC EDGAR's public database using the \texttt{edgartools} library to extract financial data from 10-K filings. All financial ratios, metrics, and M\&A analysis calculations are computed programmatically, ensuring accuracy and consistency. CSV files are automatically converted to LaTeX table format, maintaining consistent formatting across all tables in the document.

\vspace{0.3cm}
\noindent\textbf{Code Access:} The complete codebase can be accessed at \url{https://github.com/joshuagao7/CompsandMAanalysis.git}
